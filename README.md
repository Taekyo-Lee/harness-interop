...
- **OS** — Linux / macOS / WSL. hook 이 bash 스크립트라 native Windows 는 지원하지 않습니다.
...
터미널(비대화형)에서는 `claude plugin` CLI 가 같은 일을 합니다 — repo 의 `bash install-cc-plugins.sh` 를 실행하거나, 직접:
...
설치하면 memory 동기화 hook 이 걸립니다. **매 턴이 끝날 때마다** (그리고 세션 시작·종료 때) 이 프로젝트의 CC memory 가 `.opencode/from-claude.md` 로 flatten 됩니다 — 그래서 `Ctrl-C` 로 세션을 끊어도 마지막 완료 턴까지는 이미 동기화돼 있고, 혹시 남은 게 있어도 다음 실행 시작 때 따라잡습니다 (멱등).
...
repo 를 clone 했다면 `bash install-oc-plugins.sh` 가 같은 일을 해줍니다 (체크박스로 플러그인 선택 + project/global 위치 선택).
...
> **참고 — 처음 열 때 `.opencode/node_modules` (~60MB) 가 생깁니다.** 이 플러그인의 의존성이 아닙니다 (플러그인 자체는 단일 파일, 0 dependency). OpenCode 가 `.opencode/` 디렉토리가 있는 모든 프로젝트에 플러그인 SDK(`@opencode-ai/plugin`)를 자동 설치하는 자체 동작이라, 이 플러그인을 쓰지 않아도 `.opencode/` 가 있으면 동일하게 생깁니다. OpenCode 가 `.opencode/.gitignore` 를 만들어 스스로 가리므로 repo 는 오염되지 않고, 지워도 다음 실행 때 재생성됩니다.
