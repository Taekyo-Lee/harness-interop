...
> 여러 AI 코딩 harness 를 함께 쓸 때, **프로젝트 한정 개인 메모리**를 harness 사이에서 자동으로 동기화하는 플러그인 묶음.

한쪽 harness 에서 남긴 개인 메모리가 다음 세션엔 다른 harness 에도 가 있습니다. 파일은 제자리에 두고 내용만 건너가며, 오가는 파일이 모두 git 추적 밖이라 팀 repo 엔 새지 않습니다.

**현재 지원:** Claude Code ↔ OpenCode (양방향).
**예정:** Codex, Gemini CLI 등 — 같은 clobber-safe 패턴으로 harness 를 늘려갑니다.
...
/plugin install memory-bridge-claude@multi-harness-plugins
```
...
curl -fsSL \
  https://raw.githubusercontent.com/Taekyo-Lee/multi-harness-plugins/main/opencode/plugin/memory-bridge-opencode.ts \
