...
async function syncToCC(directory: string): Promise<void> {
  ...
  if (!ccMemoryDir) {
    // Fallback: probe .claude-fork then .claude
    const slug = directory.replace(/[^A-Za-z0-9]/g, "-")
    const candidates = [
      join(homedir(), ".claude-fork", "projects", slug, "memory"),
      join(homedir(), ".claude", "projects", slug, "memory"),
    ]
    for (const candidate of candidates) {
      try { await access(candidate); ccMemoryDir = candidate; break } catch { /* try next */ }
    }
    if (!ccMemoryDir) ccMemoryDir = candidates[0]  // default to .claude-fork even if absent
